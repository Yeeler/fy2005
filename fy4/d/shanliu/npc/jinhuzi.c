...
void create()
{
	set_name("½ğ´óºú×Ó", ({ "jin huzi", "jin"}) );
	set("title","½ğºú×Ó¶Ä·» ÀÏ°å");
        set("gender", "ÄĞĞÔ" );
	set("age", 48);
	set("long",
"ËûÊÇ¸öÒÂÉÑ´©µÃºÜ»ªÀöµÄ±ëĞÎ´óºº£¬Å¨Ã¼´óÑÛ£¬ÂúÁ³ºáÈâ£¬ºú×Ó¹ÎµÃ¸É¸É¾»¾»¡£\n");
...
void greeting(object ob)
{
	...
	{
		message_vision(HIY"$N¹ş¹ş´óĞ¦µÀ£º²»´í£¬²»´í£¬ÌìÉ±µÄÍºÂ¿Å£±Ç×ÓÇîËá£¬ËÀÁË¸É¾»¡£\n 
     $NÅÄÁËÅÄ$nµÄ¼ç°ò£¬ËµµÀ£º¸ÉµÃºÃ£¬ÕâÀïÊÇÄãµÄ±¨³ê¡£\n"NOR,this_object(),ob);
		gold=new("/obj/money/gold");
		gold->set_amount(ob->query_temp("É½Á÷/Æ«Ìü"));
		gold->move(ob);
		message_vision("$NË¬¿ìµØ¸ø$nÒ»Ğ©½ğ×Ó¡£\n",this_object(),ob);
	...
	{
		message_vision(HIY"$NÏò$n²»ÄÍ·³µÀ£ºÄúµ¹ÊÇ¿ìµã°¡£¬±ğÄ¥Ä¥²ä²äÁË¡£\n"NOR,this_object(),ob);
       ...
       switch( random(10) )
		{
                case 0:
                        say( "½ğºú×ÓĞ¦ßäßäµØËµµÀ£ºÕâÎ»" + RANK_D->query_respect(ob)
                                + "£¬¹ıÀ´¶ÄÁ½ÊÖ£¬¿ª¿ªĞÄ°É¡£\n");
                        break;
                case 1:
                        say( "½ğºú×Ó»ÎÁË»ÎÊÖÖĞµÄ÷»×Ó£¬½ĞµÀ£ºÕâÎ»" + RANK_D->query_respect(ob)
                                + "£¬Ğ¡¶Ä¿ÉÒÔÑø¼Òºı¿Ú£¬´ó¶Ä¿ÉÒÔ¶¨¹ú°²°î¡£À´°É£¡\n");
                        break;
                case 2:
                        say( "½ğºú×ÓËµµÀ£ºÕâÎ»" + RANK_D->query_respect(ob)
                                + "£¬½øÀ´£¡½øÀ´£¡ ÊäÁËËãÎÒµÄ£¡\n");
                        break;
        }
		return;
...
int do_answer(string arg)
{
	...
	{
		tell_object(me, 
HIR"½ğºú×Ó¶ÔÄãÇÄÉùËµµÀ£º´óÏÀÄúÏë±ØÒ²ÖªµÀ£¬ÔÛ¶Ä·»Àï×î¼É»äÊ²Ã´ºÍÉĞÄá¹ÃÁË£¬
ËùÎ½Ò»¼û¹âÍ·£¬·ê¶Ä±ØÊä£¬ÕâÑù²»°Ñ´óÒ¯ÃÇÈ«ÏÅÅÜÁË£¬´óÒ¯ÄúµÄÔËÆøÖ®ËùÒÔß¯
²î¡£¡£¡£ÔÚÆ«ÌüÀïÆ«¾ÍÓĞ¼¸¸öºÍÉĞµÀÊ¿£¬É·ÊÇÌÖÑá£¬Ö»ÒªÄã°ÑËûÃÇ¸Ï×ß¡£¡£»ò
Õß¸É´à³¬¶ÈÁËËûÄÌÄÌµÄ£¬¹ş¹ş£¬Ê²Ã´¶¼ºÃËµÀ²¡£\n"NOR);
...
ÊÇ²»ÊÇÏë·­±¾°¡¡£ÕâÓĞ¼şºÃÊÂ£¬Ö»ÒªÄã×ö³ÉÁË£¬²»½ö°ÑÄãÅâµÄËÍ»¹£¬½ğºú×ÓÎÒ
»¹»áËÍÉÏ»Æ½ğ°ÙÁ½.²»ÖªÄã¿Ï²»¿Ï×ö(answer yes or no)£¿\n"NOR,this_object(),me);
	//add_action("do_answer", "answer");	
    ...
    for(i=0; i<sizeof(inv); i++) 
	{
		if( userp(inv[i]) && (int) inv[i]->query("gamble/amount") )
		{
            ...
            if(inv[i]->query_temp("É½Á÷/ºÀ¶Ä")>=20 && inv[i]->query_temp("É½Á÷/ºÀ¶Ä")<=60)
				message_vision(HIY"½ğ´óºú×Ó¹Å¹ÖµØ¶Ô$NĞ¦ÁËĞ¦µÀ£ºÔÙÏÂÒ»×¢£¬Ëµ²»¶¨ÓĞÊ²Ã´ºÃÔËµÀßÏ¡£\n"NOR,inv[i]);
...
int accept_object(object who, object ob)
{
	...
	tell_object(who,"½ğºú×ÓÇÄÇÄ¸æËßÄã£ºÏÂÊÖ¶ùÑ¹"+message+"£¬Ò»¶¨Ó®£¡\n");
	message_vision("$N»áĞÄµÄÎ¢Ğ¦ÆğÀ´£®£®\n",who);
	
                        return 1;
                } else {
                    say("½ğºú×ÓËµµÀ£ºÕâµãÇ®Äã»¹ÊÇ×Ô¼ºÓÃ°É£¡\n");
                        return 0;
                }
        return 0;
}
