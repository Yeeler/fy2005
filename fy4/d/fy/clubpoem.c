...
void create ()
{
    set ("short", "ÇàÔÆÂ¥");
    set ("long", @LONG
µÇÉÏÇàÔÆÊ«Éç¶şÂ¥£¬·çÔÆ³ÇµÄ·±»ª¾°Ïó¾¡ÊÕÑÛµ×£¬¶«´°ÍâÊÇÌìÌ³£¬²»Ê±ÓĞ½ø¹¬
µÄ³µÂíĞĞ¹ı¡£ÕâÀïÓÀÔ¶ÊÇÈÈÈÈÄÖÄÖµÄ£¬½õÒÂµÄ¹«×Ó¸çÃÇ¶ù£¬²¼ÒÂµÄËáĞã²ÅÃÇ£¬¶¼Ï²
»¶¾ÛÔÚÕâÀïÆ·²èÒ÷Ê«£¬Ì¸¹ÅÂÛ½ñ£¬¾ÍÁ¬¶Ë²èËÍ²ËµÄĞ¡¶ş¸çÒ²ÄÜ³ö¿Ú³ÉÕÂ¡£×î½üÕâ¶ù
À´ÁË¸öĞã²Å£¬×îÏ²»¶ÓëÈË¶ÔÊ«£¬»¹ÔÚ¹ñÌ¨ÉÏĞÂÌùÁËÕÅ[33mÆôÊÂ[32m¡£
LONG);
    set("exits",  ([  /*  sizeof()  ==  1  */
	"down"  :  __DIR__"shishe",
      ]));
...
void do_test1()
{
	tell_room(this_object(), CYN"ËáĞã²ÅÒ¡Í·»ÎÄÔÒ÷µÀ£ºÊÅË®Öğ²ĞÔÂ£¬Ê±É°ÎŞÓë¶à¡£\n"NOR);
	call_out("do_test",10);
}
...
{ 
    ...
    if(!arg) return notify_fail("ÄãÏë»Ø´ğÊ²Ã´£¿\n");
    if(me->is_busy())
	return notify_fail("Äã»¹ÊÇÏÈ°ÑÊÖÍ·µÄÊÂ¶ùÃ¦Íê°É¡£\n");
    if(me->query_temp("poemok")>50 && me->query("timer/last_poem")+ 300 > time() )
	return notify_fail("ËáĞã²ÅËµ£º¸óÏÂ±¥¶Á¾­Êé£¬Ñ§¸»Îå³µ£¬ÇëĞİÏ¢Æ¬¿Ì£¬ÎÒÔÙÏë¼¸¸öÄÑµÄ£¡!\n");
...
//	me->set("timer/annie_poem_failed",time());
    ...
    /*   	if( me->query("gin")<me->query("max_gin")/20 
	   ...
	   if(living(me) ) 
		   me->receive_damage("gin",5+random(15),"¶ÁÊé¹ı¶ÈĞÄÁ¦½»´á±©×ä¡£");*/
	...
	last=replace_string(last,"£¿","");
	    ...
	    message_vision("ËáĞã²ÅÒ¡ÁËÒ¡Í·£ººÃÏó²»¶Ô°É£¿\n",me);
		if (!random(3))
	...
	write("ËáĞã²Å²»ÔÚ£¬Ã»ÓĞÈË²ÃÅĞ¶Ô´íÁË£®£®£® £º£¨\n");
    }
    return 1;
}
void poem_penalty(object me)
{
	object r;
    message_vision(HIW"ËáĞã²ÅÒ¡Í·µÀ£ºÖªÖ®ÎªÖªÖ®£¬²»ÖªÎª²»Öª£¬·½ÊÇÖªÒ²¡£\n",me);
    message_vision(HIR"ËáĞã²ÅµÀ£ºÈ¥£¬È¥£¬È¥¡£\n",me);
    message_vision(MAG"ËáĞã²Å»Ó»ÓĞä×Ó£¬$NÌÚÔÆ¼İÎí°ãµØ´ÓÇàÔÆÂ¥ÉÏ·ÉÁË³öÈ¥¡£\n"NOR,me);
    me->set_temp("poem/wrong",0);
	r=find_object("/d/fy/jssju2");
	if (!r)
		r=load_object("/d/fy/jssju2");
	me->move(r);
    message_vision(HIG"Ö»ÌıµÃ»©À²Ò»Éù£¬Ò»¸öÈËÓ°×²¿ªÁË¶¥Â¥µÄÖ½´°£¬ÖØÖØµØÔÒÔÚÇ½ÉÏ¡£\n"NOR,me);
	me->unconcious();
	me->set("timer/annie_poem_failed",time());
    return;
}
...
{
    me->set_temp("poem/wrong",0);
    message_vision(CYN"ËáĞã²ÅµãÍ·µÀ£º´ğ¶ÔÁË£¬²»´í£¡²»´í£¡\n"NOR,me);
...
//	current=replace_string(quest,"£¬","");  // the answer
	current=quest;	// ÒªÇó±êºÅÎ»ÖÃÕıÈ·
	quest=mixup(quest);   // the question
	quest=mixup(quest);   // the question
	curr_show=quest;
	tell_room(this_object(), enscript("ËáĞã²ÅÌá±ÊÔÚÇ½ÉÏĞ´µÀ£º"+quest)+"\n");
    }
...
int do_look(string arg){   
	...
	if (wizardp(this_player()))
		write(CYN"µ±Ç°µÄ´ğ°¸ÊÇ£º"+current+"\n"NOR);
//	for (i=0;i<length1;i++)	    write("    "+poem1[i]+"\n");
	...
	write("\n"+@LONG
           ±¾ÉçĞÂÔö²ÂÊ«ÓÎÏ·£¬ÓÉËáĞã²Å½«Ò»¾äÊ«´ÊµÄÈô¸É×Ö
       µßµ¹´ÎĞò£¬Ğ´ÔÚÇ½ÉÏ¡£ÄÜ×¼È·´ğ³ö(answer)Ô­¾äÕßÎªÊ¤¡£
       ÒªÏëÖªµÀµ±Ç°ÌâÄ¿£¬¿ÉÒÔÍùÇ½ÉÏ(wall)¿´¡£
       ÀıÈç£ºËáĞã²ÅÌá±ÊÔÚÇ½ÉÏĞ´µÀ£ºÀëÀëÔ­ÉÏ²İÒ»ÈÙÒ»¿İËê
