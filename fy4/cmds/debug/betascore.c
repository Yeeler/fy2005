...
int main(object me, string arg)
{
	...
	line =  sprintf(" ¡¾Á¦Á¿¡¿%-8s\t¡¾Ãñ×å¡¿%-10s¡¾É±ËÀÈËÊý¡¿ %-12d \n",
	           display_attr(my["str"], ob->query_str()),(ob->query("national")? ob->query("national") : "ÎÞ"),my["MKS"]
	        );
	line += sprintf(" ¡¾ËÙ¶È¡¿%-8s\t¡¾ÐÔ±ð¡¿%-10s¡¾É±ËÀÍæ¼Ò¡¿ %-12d \n",
	           display_attr(my["agi"], ob->query_agi()),ob->query("gender"),my["PKS"]
	        );
	line += sprintf(" ¡¾ÌåÖÊ¡¿%-8s\t¡¾ÄêÁä¡¿%-10s¡¾±»É±´ÎÊý¡¿ %-12d \n",
	           display_attr(my["con"], ob->query_con()),chinese_number(ob->query("age")),my["KILLED"]
	        );
	line += sprintf(" ¡¾²ÅÖÇ¡¿%-8s\t¡¾ÃÅÅÉ¡¿%-10s¡¾É±ËÀÌì½¾¡¿ %-12d \n",
	           display_attr(my["int"], ob->query_int()),
	           (my["family"]?(my["family"]["family_name"]? my["family"]["family_name"]:"ÎÞ"):"ÎÞ")
	           + (my["betrayer"]? sprintf("*%d",my["betrayer"]):""),
	           sizeof(my["m_killer"])
	        );
	line += sprintf(" ¡¾¶¨Á¦¡¿%-8s\t¡¾Ê¦¸¸¡¿%-10s¡¾£Ô£Á£Ó£Ë¡¿ %-12d \n",
	...
	if ( !my["marry"]) 
	{
		line += sprintf(" ¡¾ÔËÆø¡¿%-8s\t¡¾»éÒö¡¿%-10s¡¾£Ñ£Ó£Ô¡£¡¿ %-12d \n",
	...
	{
		line += sprintf(" ¡¾ÔËÆø¡¿%-8s\t¡¾%s¡¿%-10s¡¾£Ñ£Ó£Ô¡£¡¿ %-12d \n",
	           display_attr(my["kar"], ob->query_kar()),
	           (ob->query("gender") == "Å®ÐÔ" ? "ÕÉ·ò":"ÆÞ×Ó"),
	            my["marry_name"],
	            my["FINISH_QUEST"]		
	        );
	}
...
¡¡ÓÐ¹ØÊôÐÔ£¨²ÅÖÇ£¬Á¦Á¿µÈ£©£º²Î¼û Help newbie
 ...
 [37m¡¾ÕÐ¼Ü¡¿[32m¶Ô³£¹æÎä¹¦¹¥»÷µÄÕÐ¼ÜÄÜÁ¦£¬Èô×°±¸ÎäÆ÷£¬ÔòÓÃ¸ÃÎäÆ÷ÕÐ¼Ü£¬
 	 ...
 	 £å£î£á£â£ì£å£©ºÍÉíÌå×´¿ö¡£
 
 [37m¡¾ÉÁ¶ã¡¿[32mÓë¡¾ÕÐ¼Ü¡¿ÀàËÆ£¬¶Ô³£¹æÎäÆ÷¹¥»÷µÄÉÁ±ÜÄÜÁ¦£¬È¡¾öÓÚ¾­ÑéµÈ
 	 ¼¶¡¢ÓÐÐ§ÉÁ±Ü¼¼ÄÜ£¨£ä£ï£ä£ç£å¡¡£å£î£á£â£ì£å£©ºÍÉíÌå×´¿ö¡£ 		  
 
 [37m¡¾»¤¼×¡¿[32m´©´÷µÄÒÂÎï¿ø¼×µÈ£¬Ò»°ãÀ´Ëµ£¬»¤¼×ÄÜÖ±½Ó¼õÉÙ»òµÖÏûµÐÈËµÄ
 	 É±ÉËÁ¦ 
 
 [37m¡¾»¤·¨¡¿[32m·çÔÆÖÐ³ýÁË³£¹æµÄÎä¹¦ÕÐÊ½Íâ£¬»¹ÓÃ·á¸»¶à²ÊµÄÄ§·¨ÖäÓïµÈ·Ç
 	 ³£¹æÎä¹¦£¬ÈçÈýÇåµÄ²ÔÁéÖ®¼ý£¬µÛÍõ¹ÈµÄÑª½µ¡¢¿ì»îÁÖµÄ²ÐÔÆ
 	 ±ÌµÈ¡£ÕâÐ©·Ç³£¹æÎä¹¦µÄ·ÀÓùÁ¦ÓëÇ°ÃæÌáµ½µÄ¡¾ÕÐ¼Ü¡¿¡¢¡¾ÉÁ
 	 ¶ã¡¿ÎÞ¹Ø£¬¶øÊÇÈ¡¾öÓÚÄãµÄ¾­ÑéµÈ¼¶ºÍÁéÊ¶ÄÜÁ¦
 	£¨£ð£å£ò£ã£å£ð£ô£é£ï£î£©£¬Ò²ÓëÄãºÍµÐÊÖµÄÏà¶Ô²ÅÖÇÏà¹Ø¡£
...
[37m¡¾£Ñ£Ó£Ô¡£¡¿[32m¡¡Ö¸£Ñ£õ£å£ó£ô£¬·çÔÆÂÒÊÀ£¬Ìì»úÀÏÈË¼±ÐèÓÐÖ¾Ö®Ê¿Ð­Öú£¬
	...
	¡¡À´Ô½ÄÑ¡£¡£¡£¡££Ñ£õ£å£ó£ôÊÇ·çÔÆÔö¼ÓÎä¹¦¾­ÑéµÄÒ»´óÍ¾¾¶¡£
	
[37m¡¾£Ô£Á£Ó£Ë¡¿[32m²Î¿´£è£å£ì£ð¡¡£ô£á£ó£ë£é£î£ç£®£ô£ø£ô
