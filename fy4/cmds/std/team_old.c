...
//		ÑûÇëÈë¶Ó¡£
		...
		//	(1) ¼ì²éÊÇ·ñºÏ·¨ 	
			if( ob->query("quest"))
				return notify_fail(WHT"¶Ô·½»¹ÓĞÃ»Íê³ÉµÄÈÎÎñ£¬²»ÄÜ×é¶Ó£¡\n"NOR);
			...
			// If Me is in a team
				if (!me->is_team_leader())
					...
					while (num--) {
						if (t[num]->query("quest") && max_exp < ob_exp)
							return notify_fail(WHT"¶ÓÎéÖĞµÄÈÎÎñµÍÓÚ¼ÓÈëÕß¾­Ñé£¬ĞèÒªÏÈÍê³ÉÈÎÎñ¡£\n"NOR);
						if (n*t[num]->query("combat_exp")<ob_exp 
							|| t[num]->query("combat_exp")+ 2000000< ob_exp)
							return notify_fail(WHT"×é¶Ó¾­Ñé·¶Î§²î±ğÌ«´ó¡£\n"NOR);
				...
				if( me->query("quest"))
					return notify_fail(WHT"Äã»¹ÓĞÃ»Íê³ÉµÄÈÎÎñ£¬²»ÄÜ×é¶Ó£¡\n"NOR);
		...
		//	AÑûÇëB£¬ÒÔÉÏÖ¤ÊµA¼°Æä¶ÓÎé£¬ÒÔ¼°B±¾ÉíÊÇºÏ·¨µÄ£¬µ«ÊÇ²»¼ì²éBµÄ¶ÓÎé¡£
				
		//	µ±BÔÙ´ÎÊäÈëteam with Ê±£¬A/BÒÔ¼°BµÄ¶ÓÎé»áÔÙ´ÎÍ¨¹ıÉÏÊö¼ì²é£¬
		//	µ«ÊÇAµÄ¶ÓÎé£¬ÒÔ¼°BÊÇ·ñ¼ÓÈëÆäËû¶ÓÎéÖĞ¿ÉÄÜÒÑÓĞ±ä¶¯£¬ĞëÔÚÏÂÃæ¸´²é¡£	
		
			if( !pointerp(me->query_team()) || me->is_team_leader() ) {
				if( me==(object)ob->query_temp("pending/team") ) {
					//	¼ì²éA¶ÓÎéÊÇ·ñºÏ·¨¡£·Ç¶ÓÎéµÄÇé¿öÒÑ¾­ÔÚÉÏÃæ¼ì²é¹ıÁË¡£
					if (pointerp(t1=ob->query_team())) {
						// If Ob is in a team
						if (me->query_team())
							return notify_fail(WHT"ÄãÒÑ¾­ÔÚÄ³¸ö¶ÓÎéÖĞÁË¡£\n"NOR);				
						if (!ob->is_team_leader())
							...
							while (num--) {
								if (t1[num]->query("quest") && max_exp < me->query("combat_exp"))
									return notify_fail(WHT"¶Ô·½¶ÓÎéÖĞµÄÈÎÎñµÍÓÚÄãµÄ¾­ÑéÖµ£¬ÔİÊ±²»ÄÜ¼ÓÈë¡£\n"NOR);
...
×¢: team ¸ú follow ÊÇ¶ÀÁ¢µÄ£¬Äã²»Ò»¶¨Òª follow ¶ÓÎéµÄÁìĞä¡£

×¢£º×é¶ÓÊ±¾­ÑéÖµ²»ÄÜÏà²îÌ«´ó£¬ĞÂ¼ÓÈëÕß²»ÄÜÓĞÈÎÎñÔÚÉí£¬
	Èç¹û¶ÓÎéÖĞ×î¸ß¾­ÑéÖµ´óÓÚĞÂ¼ÓÈëÕß£¬Ôò¶ÓÎéÖĞÒ²²»ÄÜÓĞÈÎÎñ¡£
